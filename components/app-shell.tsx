  ...
  return (
    <div className="min-h-full lg:grid lg:grid-cols-[16rem_minmax(0,1fr)]">
      <aside className="bg-sidebar text-sidebar-foreground lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col lg:border-r lg:border-sidebar-border">
        <header className="flex h-16 items-center justify-between border-b border-sidebar-border px-4 lg:h-auto lg:px-6 lg:py-6">
          <BrandMark size="md" />
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 order-last grid grid-cols-4 border-t border-sidebar-border bg-sidebar px-2 pb-[env(safe-area-inset-bottom)] lg:static lg:mt-6 lg:flex lg:flex-col lg:border-0 lg:bg-transparent lg:px-3" aria-label="Primary">
        ...
        <div className="mt-auto hidden px-4 pb-5 lg:block">
          <div className="rounded-xl border border-sidebar-border bg-sidebar-accent p-4">
            ...
            <p className="mt-3 text-sm font-semibold text-sidebar-foreground">Audit-ready by design</p>
            <p className="mt-1 text-xs leading-5 text-[hsl(218,14%,72%)]">Every match keeps its evidence and decision trail.</p>
          </div>
          <Link href="/" className="mt-3 flex min-h-10 items-center justify-between px-2 text-xs text-[hsl(218,14%,72%)] hover:text-sidebar-foreground">View website <ArrowUpRight className="h-4 w-4" /></Link>
        </div>
      </aside>
      <main className="min-w-0 px-4 pb-24 pt-8 sm:px-8 lg:px-12 lg:pb-12 lg:pt-10">
