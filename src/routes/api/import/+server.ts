...
function classify(name: string, text: string): Kind {
  ...
  const t = text.trim();
  if (n.endsWith('.json') || t.startsWith('{') || t.startsWith('[')) {
    return n.includes('truth') || truth(t) ? 'groundTruth' : 'dodo';
  }
  ...
  if (header.includes('sale_id') || header.includes('email')) return 'gumroad';
